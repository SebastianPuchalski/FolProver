...
bool NaiveSuperpositionSolver::performOccursCheck(const std::string& symbol,
    ...
    if (expr->exprType == Expression::Type::VARIABLE) {
        ...
        auto it = mgu.find(variable->symbol);
        if (it != mgu.end()) {
            return performOccursCheck(symbol, it->second, mgu);
        }
    }
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        if (performOccursCheck(symbol, child, mgu)) return true;
    }
    return false;
}
...
long NaiveSuperpositionSolver::getCurrentMemoryUsageMB() const {
#ifdef _WIN32
